...
class QueryAnalytics:
        ...
        try:
            with self._get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    # row is sqlite3.Row, so row['intent'] is valid
                ...
                total_queries = total_row['total'] if (total_row is not None and 'total' in total_row.keys()) else 0
                ...
                avg_processing_time = avg_row['avg_time'] if (
                            avg_row is not None and 'avg_time' in avg_row.keys()) else None
        ...
        try:
            with self._get_connection() as conn:
                ...
                if perf_row is not None:
                    ...
                    perf_metrics = {key: perf_row[key] for key in perf_row.keys()}
                else:
                    perf_metrics = {
                        "avg_total_time": None,
                        "min_total_time": None,
                        "max_total_time": None,
                        "avg_embedding_time": None,
                        "avg_search_time": None,
                        "avg_ranking_time": None,
                ...
                if response_times:
                    ...
                    index_95 = int(len(sorted_times) * 0.95) - 1
                    index_95 = max(0, min(index_95, len(sorted_times) - 1))
                ...
                avg_results = avg_res_row['avg_results_count'] if (
                            avg_res_row is not None and 'avg_results_count' in avg_res_row.keys()) else None
    ...
    def generate_performance_report(self, time_period: str = "day") -> Dict[str, Any]:
        ...
        # Determine lookback window and pandas resample frequency
        now = int(time.time())
        if time_period == "hour":
            lookback = now - 3600
            freq = "5T"  # 5 minutes
        ...
        try:
            with self._get_connection() as conn:
                ...
                if not df.empty:
                    ...
                    # Resample by appropriate frequency
                    query_volume = df["timestamp"].resample(freq).count()

                    # Get success rate over time
                    success_rate_ts = df["status"].resample(freq).apply(
                        lambda x: (x == "success").mean() if len(x) > 0 else 0.0
                    )
                else:
                    # If there are no rows, return empty Series/dicts
                    query_volume = pd.Series(dtype=int)
                    success_rate_ts = pd.Series(dtype=float)
                ...
                top_queries: List[tuple] = [(row['query_text'], row['count']) for row in cursor.fetchall()]
                ...
                avg_rating = avg_rating_row['avg_rating'] if (
                            avg_rating_row is not None and 'avg_rating' in avg_rating_row.keys()) else None
                ...
                return {
                    ...
                    "success_rate_over_time": success_rate_ts.to_dict(),
