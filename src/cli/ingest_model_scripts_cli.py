...
def _format_natural_date(iso_date: str):
...
def _process_and_store_chunks(chunks, model_id, metadata_doc_id, schema_validator, text_embedder,
                              chroma_manager):
...
def _extract_llm_fields(parse_result):
...
def _validate_and_store_metadata_documents(schema_validator, metadata_documents, text_embedder,
    ...
    for key, value in metadata_documents.items():
        ...
        for document in documents:
            ...
            try:
                embedding_content = _create_metadata_content_for_type(collection_name, document)
...
def _extract_and_prepare_metadata(metadata_extractor, parse_result, file_path):
    ...
    last_modified_raw = _clean_iso_timestamp(metadata.get("file", {}).get("last_modified_date", "N/A"))

    creation_natural_month = _format_natural_date(creation_date_raw)
    last_modified_natural_month = _format_natural_date(last_modified_raw)

    # Extract additional metadata from LLM parse_result
    llm_fields = _extract_llm_fields(parse_result)
    ...
    for i, chunk in enumerate(chunk_descriptions):
        ...
        metadata_documents["model_descriptions"].append({
            ...
            "metadata": {
                ...
                "offset": chunk['offset'],
                "description": chunk['description'] or "No description available"
            }
        })
...
class ScriptProcessorRunner:
    ...
    def process_model_scripts(self, components, directory_path):
        ...
        # Find all model model_script files recursively
        script_files = []
        ...
        logger.info(f"Found {len(script_files)} potential model model_script files after filtering")
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    ...
                    if result:
                        ...
                        logger.warning(f"Skipped {file_path}: Not a model model_script")
                except Exception as e:
        ...
        logger.info("Model model_script processing completed")
    ...
    def process_single_script(self, components, file_path, logger=None):
        ...
        text_embedder = components["vector_db"]["text_embedder"]
        ...
        # Extract metadata and prepare metadata documents for different tables
        model_id, metadata_documents = _extract_and_prepare_metadata(
            metadata_extractor,
            parse_result,
            file_path
        )
        ...
        # Validate and store metadata documents in different tables
        metadata_stored = _validate_and_store_metadata_documents(
            schema_validator,
            metadata_documents,
            text_embedder,
            chroma_manager,
        ...
        chunk_documents = _process_and_store_chunks(
            chunks,
            model_id,
        ...
        if chunk_documents:
            ...
            logger.warning(f"Skipped {file_path}: Failed to process model model_script")
            return None, False
